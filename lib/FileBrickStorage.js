...
const util = require('../utils/utilConstants.js');
const utilFunctions = require('../utils/utilFunctions.js');
const ensuresFileDoesNotExists = new utilFunctions().ensureFileDoesNotExists;
const constantsUtil = new util();
...
function FileBrickStorage(location){
    ...
    function firstWriteToFile(fd,brick,callback){
        ...
        fs.write(fd, tempBuffer, 0, tempBuffer.length, constantsUtil.getSaltSize(), (err, wrt, buffer) => {
            if (err) {
                return callback(err);
            }
            fs.close(fd,(err)=>{
    ...
    function getSalt(fd,callback){
        let numberBuffer = Buffer.alloc(constantsUtil.getSaltSize());
    ...
    this.getBrick = function(brickIndex,callback){
        ...
        if(brickIndex === 0){
            tempBuffer = Buffer.alloc(map.getPosition(brickIndex) - constantsUtil.getSaltSize());
        }else{
