function BrickTransform(transformGenerator) {
    ...
    this.applyDirectTransform = (data) => {
        if (!directTransform) {
            directTransform = transformGenerator.createDirectTransform();
        }
        ...
        if(directTransform.transformParameters){
            if (directTransform.transformParameters.salt) {
                transformedData = Buffer.concat([transformedData, directTransform.transformParameters.salt]);
            }
