...
function Seed(compactSeed, location, randomLength = 32) {
    let seed;
    let lseed;
    ...
    function generateCompactForm(expandedSeed) {
        ...
        if (expandedSeed.location) {
            compactSeed += '|' + Buffer.from(JSON.stringify(expandedSeed.location)).toString('base64');
        }
    ...
    function load(compactFormSeed) {
        ...
        if (splitCompactSeed[1] && splitCompactSeed[1].length > 0) {
            localSeed.location = JSON.parse(Buffer.from(splitCompactSeed[1], 'base64').toString());
        }
