...
function LatestVersionStrategy(options) {
     ...
     * @return {BrickMapDiff}
     */
    this.compactDiffs = (callback) => {
        this.brickMapState.prepareNewChangesForAnchoring((err) => {
            if (err) {
                return callback(err);
            }

            this.brickMapState.cloneAnchoredBrickMap((err, brickMap) => {
                if (err) {
                    return callback(err);
                }
                const result = this.mergeDiffs(brickMap, this.brickMapState.getDiffsInAnchoring());
     ...
     * @param {callback} callback
     */
    this.reconcile = (brickMap, callback) => {
        ...
        state.prepareNewChangesForAnchoring((err) => {
            ...
            brickMap.clone((err, brickMapCopy) => {
                ...
                const theirChanges = ourAnchoredBrickMap.diff(brickMap);

                // Check if any of our changes conflict with upstream changeset
                const conflicts = brickMapCopy.detectMergeConflicts(theirChanges, state.getDiffsInAnchoring());
                ...
                // No conflicts detected, go ahead and merge
                try {
                    const [ourChanges, mergedDiffs] = this.mergeDiffs(ourAnchoredBrickMap, state.getDiffsInAnchoring());
                    brickMapCopy.merge(ourChanges);
                } catch (e) {
                    state.rollback(mergedDiffs)
                    return callback(e);
                }
                state.setDirtyBrickMap(brickMapCopy);
                return callback();
            });
        })
