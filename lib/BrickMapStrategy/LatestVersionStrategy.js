...
function LatestVersionStrategy(options) {
     ...
     *              [remoteHashLinks]: [HashLinkSSI, ...] // only for errors of type: LOCAL_OVERWRITE
     *          },
     *          ...
     *      }
     *  }
     ...
     *  brickMapController - The DSU BrickMapController instance
    ...
    this.reconcile = (theirBrickMap, callback) => {
        ...
        state.cloneAnchoredBrickMap((err, ourAnchoredBrickMap) => {
            ...
            state.prepareNewChangesForAnchoring((err) => {
                ...
                const conflicts = theirChanges.detectMergeConflicts(state.getDiffsForAnchoring());

                // Call the conflict resolution function if it is defined, or return with error
                if (conflicts) {
                    if (typeof conflictResolutionFn === 'function') {
                        // TODO: Discuss this! Maybe pass some callbacks like: acceptTheirChanges(), acceptOurChanges()
