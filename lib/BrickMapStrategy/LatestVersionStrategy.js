...
function LatestVersionStrategy(options) {
     ...
     * @param {boolean} mergeNewDiffs False when merging upstream changes and DSU is in batch mode
     *                                True in any other case
     * @param {function} callback
     */
    this.reconcile = (theirBrickMap, ourHashLinkSSI, mergeNewDiffs, callback) => {
        if (typeof mergeNewDiffs === 'function') {
            callback = mergeNewDiffs;
            mergeNewDiffs = true;
        }
        ...
        state.cloneAnchoredBrickMap((err, ourAnchoredBrickMap) => {
            ...
            state.prepareNewChangesForAnchoring((err) => {
                ...
                if (!mergeNewDiffs) {
                    // When DSU operates in batch mode
                    // and received upstream changes via subscription notification
                    // we only need to detect conflicts, the diffs will be merged
                    // when commiting the batch
