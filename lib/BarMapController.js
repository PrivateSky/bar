...
function BarMapController(options) {
    ...
    const initialize = () => {
        ...
        const barMapProperties = Object.getOwnPropertyNames(validBarMap);

        for (const property of barMapProperties) {
            if (typeof validBarMap[property] !== 'function') {
                continue;
            }
    ...
    const createProxyMethod = (method) => {
        const proxy = new Proxy(function () {}, {
            apply: (target, thisArg, argumentsList) => {
                ...
                if (sessionBarMap) {
                    console.log('using session bar for ' + method);
                    targetBar = sessionBarMap;
                } else {
                    console.log('using bar for ' + method);
                }
    ...
    this.saveSession = (sessionBarMap, callback) => {
        ...
        saveBarMapBrick(diffBarMap, (err, hash) => {
            ...
            brickStorageService.updateAlias(alias, hash, (err) => {
                ...
                validBarMap = sessionBarMap.getBarMap();
