...
function CommandExecuter(commandObject) {
    function __call() {
        ...
        } else if (commandObject.getCommand() === 'x' || commandObject.getCommand() === 'xf' || commandObject.getCommand() === 'xzf' || commandObject.getCommand() === 'xvf') {
...
function executeXvf(storageProvider, parameters, config) {
    ...
    if (fs.statSync(parameters[0]).isFile() === true) {
        ...
        archive.extractFile(parameters[0], location, (err) => {
            if (err) {
                throw err;
            }
