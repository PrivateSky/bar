...
function CommandHandler(argv) {
    ...
    function parse() {
        constructList();
        const commandIsValid = checkIfCommandIsValid(flagList, parametersList);

        if (commandIsValid === true) {
            constructCommand();
            createConfigurator();
        }
    }
    ...
    function setStorageProvider(value) {
        ...
        let suffix = value.slice(value.length - 3);
        if (suffix === 'bar') {
    ...
    function getSavePath() {
        if (actualCommand === 'czf' || actualCommand === 'cf' || actualCommand === 'cfz' || actualCommand === 'cvf') {
    ...
    function helperConstructList(arg) {
        if (arg[0] === '-') {
            if (arg[1] === '-') {
                situationalConfigurator(arg.toString().split('='));
            } else {
                flagList.push(arg.toString());
            }
        } else {
            if (parametersList.length === 0) {
                setStorageProvider(arg.toString());
            }
            parametersList.push(arg.toString());
        }
    }
...
CommandHandler.prototype.provideHelp = function () {
    ...
    CommandHandler.prototype.aliasList.forEach(aliases => {
        if (maximumLeng < aliases.toString().length) {
            maximumLeng = aliases.toString().length;
        }
    });
    CommandHandler.prototype.aliasList.forEach(aliases => {
        let tempAliases = aliases.toString();
        while (tempAliases.length < maximumLeng + 3) {
            tempAliases += ' ';
        }
        helpString += tempAliases.toString();
...
CommandHandler.prototype.registerFlag(['-v'], 'boolean', createFunction(['-f', '-c', '-u', '-r', '-t', '-z', '-B']), 'display every file name from data flux', 0);
