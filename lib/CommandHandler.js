...
function CommandHandler(argv) {
    ...
    function createConfigurator() {
        ...
        ArchiveConfigurator.prototype.registerFsAdapter(diskAdapter, createFsAdapter);
        archiveConfigurator = new ArchiveConfigurator();
        archiveConfigurator.setStorageProvider(storageProvider, savePath);
        archiveConfigurator.setFsAdapter(diskAdapter);
        archiveConfigurator.setBufferSize(bufferSize);
        archiveConfigurator.setMapEncryptionKey(encryptionKey);
    }
...
CommandHandler.prototype.registerFlag(['-c', '--create'], 'String', createFunction(['-z', '-v', '-f', '-B', '-K']), 'create a new archive', 1);
...
CommandHandler.prototype.registerFlag(['-u', '--update'], 'String', createFunction(['-f', '-v', '-B', '-k']), 'only append files newer than copy in archive', 1);
CommandHandler.prototype.registerFlag(['-x', '--extract', '--get'], 'String', createFunction(['-z', '-v', '-f', '-B', '-K']), 'extract files from an archive', 1);
CommandHandler.prototype.registerFlag(['-C', '--directory'], 'String', undefined, 'change to directory give as parameter', 1);
CommandHandler.prototype.registerFlag(['-f', '--file'], 'String', createFunction(['-z', '-x', '-v', '-B', '-K']), 'use archive file or device ARCHIVE', 1);
...
CommandHandler.prototype.registerFlag(['-z', '--gzip'], 'boolean', createFunction(['-x', '-f', '-c', '-t', '-v', '-B', '-K']), 'filter the archive through gzip', 0);
CommandHandler.prototype.registerFlag(['-v'], 'boolean', createFunction(['-f', '-c', '-u', '-r', '-t', '-z', '-B', '-K']), 'display every file name from data flux', 0);
...
CommandHandler.prototype.registerFlag(['-K', '--key'], 'String', createFunction([]), 'allows user to encrypt the archive with the specified encryption key', 1);
