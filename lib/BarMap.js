...
function BarMap(header) {
    ...
    this.applyDiff = (diffBarMap) => {
        const diffFilesList = diffBarMap.getFileList('/', true);
        const fileFiles = this.getFileList('/', true);
        // TODO: implement
    }
    ...
    this.clone = () => {
        ...
        const clonedHeader = JSON.parse(JSON.stringify(header));
        const barMapClone = new BarMap(clonedHeader);
        barMapClone.setConfig(archiveConfig);
        barMapClone.setEncryptionKey(encryptionKey);
        barMapClone.load();
