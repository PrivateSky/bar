...
function FileBarMap(header) {
    ...
    this.add = function (filePath, brick) {
        ...
        const additionalTransformData = brick.getAdditionalTransformData();
        if(additionalTransformData){
            brickObj.key = additionalTransformData.key;
        }
    ...
    this.getAdditionalTransformData = function (brickId) {
        let bricks = [];
