...
const BrickMapStrategyFactory = require('./BrickMapStrategy').Factory;
...
function BrickMapController(options) {
    ...
    this.getAnchors = (alias, callback) => {
        brickStorageService.getAnchors(alias, callback);
    }
     ...
     * @param {callback} callback
     */
    this.updateAnchor = (alias, hash, lastHash, callback) => {
        brickStorageService.updateAnchor(alias, hash, lastHash, callback);
    }
    ...
    this.anchorChanges = (listener) => {
        ...
        this.saveBrickMap(brickMap, (err, res) => {
            if (err) {
            ...
            const alias = config.getBrickMapId();
            this.updateAnchor(alias, hash, lastValidHash, (err) => {
                if (err) {
