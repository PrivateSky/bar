...
function BrickMapController(options) {
        ...
        strategy.compactDiffs(brickMap, (err, result) => {
            ...
            this.saveBrickMap(bricksDomain, brickMap, (err, hash) => {
                ...
                keySSI.sign(dataToSign, (err, signature) => {
                    ...
                    const signedHashLink = keyssi.createSignedHashLinkSSI(bricksDomain, hashLink.getIdentifier(), timestamp, signature, keySSI.getVn());

                    anchoring.appendToAnchor(keySSI, signedHashLink, state.getLastAnchoredHashLink(), (err) => {
                        if (err) {
    ...
    this.handleAnchoringConflict = (listener) => {
        ...
        strategy.load(keySSI, (err, brickMap) => {
            ...
            strategy.reconcile(brickMap, currentAnchoredHashLinkSSI, (err, reconciledBrickMap) => {
                if (err) {
                    return endAnchoring(listener, anchoringStatus.BRICKMAP_RECONCILE_ERR, err);
                }
                state.setDirtyBrickMap(reconciledBrickMap);

                anchoringInProgress = false;
                this.anchorChanges(listener, reconciledBrickMap);
            });
        });
    }
