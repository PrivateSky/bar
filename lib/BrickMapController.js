...
function BrickMapController(options) {
    ...
    const builtinStrategies = require("./BrickMapStrategy/bultinBrickMapStrategies");
    options = options || {};
    ...
    this.cloneFolder = (srcPath, dstPath, callback) => {
        validator.validate('preWrite', dirtyBrickMap, 'clone', srcPath, {
            dstPath
        }, (err) => {
            if (err) {
        ...
        moveNewDiffsToPendingAnchoringState((err) => {
            ...
            strategy.compactDiffs(pendingAnchoringDiffs, (err, brickMap) => {
                ...
                this.saveBrickMap(keySSI, brickMap, (err, hash) => {
                    ...
                    if (lastValidHashLink) {
                        dataToSign = lastValidHashLink.getIdentifier() + timestamp;
                    }
                    ...
                    keySSI.sign(dataToSign, (err, signature) => {
                        ...
                        const signedHashLink = keyssi.createSignedHashLinkSSI(keySSI.getDLDomain(), hashLink.getIdentifier(), timestamp, signature, keySSI.getVn(), keySSI.getHint());
                        // TODO: call strategy.signHash() and pass the signedHash
                        this.addVersion(keySSI, signedHashLink, lastValidHashLink, (err) => {
                            if (err) {
                            ...
                            strategy.afterBrickMapAnchoring(brickMap, signedHashLink, (err, _hashLink) => {
                                if (err) {
    ...
    this.stat = (path) => {
        return validBrickMap.stat(path);
    };
