...
function State() {
    const brickMap =  {
        ...
        anchored: undefined,
        // The current BrickMap, cloned from `anchored`. Can contain un-anchored changes
        dirty: undefined,
    };
    const diffs = {
    ...
    this.canBeAnchored = () => {
        return this.hasNewDiffs() || this.hasDiffsInAnchoring();
    }

    this.getDiffsInAnchoring = () => {
    ...
    this.setAnchoredBrickMap = (_brickMap) => {
        brickMap.anchored = _brickMap;
    }
...
function BrickMapController(options) {
    ...
    const createBrickMapDiff = (callback) => {
        const brickMapDiff = new BrickMapDiff();
    ...
    const anchoringRequestExists = () => {
        return state.hasDiffsInAnchoring();
    }
    ...
    const isAliasSyncError = (err) => {
        ...
        } while (error.previousError || error.statusCode);
        return false;
    }
     ...
     */
    this.init = (callback) => {
        this.createNewBrickMap((err, brickMap) => {
            if (err) {
    ...
    this.anchorChanges = (listener) => {
        if (anchoringInProgress || !state.canBeAnchored()) {
            return listener();
        }
        ...
        // BrickMap object.
        strategy.compactDiffs((err, result) => {
            if (err) {
            ...
            this.saveBrickMap(keySSI, brickMap, (err, hash) => {
                ...
                brickStorageService.addVersion(keySSI, hashLink, state.getLastAnchoredHashLink(), (err) => {
                    if (err) {
    ...
    this.handleAnchoringConflict = (listener) => {
        strategy.load(keySSI, (err, brickMap) => {
            ...
            strategy.reconcile(brickMap, (err) => {
                if (err) {
