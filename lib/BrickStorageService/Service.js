...
function Service(options) {
     ...
     * @param {callback} callback
     */
    const convertDataBlockToUnencryptedBrick = (data, callback) => {
        const brick = this.brickFactoryFunction();
        brick.setRawData(data);

        this.putBrick(this.keySSI, data, (err, digest) => {
            if (err) {
                return callback(err);
            }
    ...
    const storeCompactedFiles = (buffer, filesList, callback) => {
        return convertDataBlockToUnencryptedBrick(buffer, (err, brickMeta) => {
            if (err) {
                return callback(err);
            }
            const files = {};
    ...
    this.createBrickFromFolder = (folderPath, callback) => {
        ...
        const iteratorHandler = (err, filename, dirname) => {
            ...
            this.readFile(filePath, (err, fileBuffer) => {
                ...
                const fileBrick = this.brickFactoryFunction();
    ...
    this.createBrickFromFiles = (files, callback) => {
        ...
        const readFilesRecursive = (files, callback) => {
            if (!files.length) {
                return storeCompactedFiles(filesList, callback);
            }
