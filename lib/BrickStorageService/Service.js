...
function Service(options) {
     ...
     * @return {HashLinkSSI}
     */
    const removeHintFromHashLink = (hlSSI) => {
        return keyssi.buildHashLinkSSI(
            hlSSI.getDLDomain(),
    ...
    const getBrickAsBuffer = (brickMeta, callback) => {
        ...
        if (hlSSIHint.indexOf(HASHLINK_EMBEDDED_HINT_PREFIX) === 0) {
            cacheKey = removeHintFromHashLink(hlSSI).getIdentifier();
        }
