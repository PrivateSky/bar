...
function Service(options) {
     ...
     *
     * @param {Buffer} data
     * @param {boolean|callback} encrypted Defaults to `true`
     * @param {callback|undefined} callback
     */
    const convertDataBlockToBrick = (data, encrypted, callback) => {
        if (typeof encrypted === 'function') {
            callback = encrypted;
            encrypted = true;
        }
        const brick = this.brickFactoryFunction(encrypted);
