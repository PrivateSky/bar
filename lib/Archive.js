...
function Archive(archiveConfigurator) {
    ...
    this.load = (callback) => {
        initialize((err) => {
            ...
            this.enableAnchoringNotifications(publishAnchoringNotifications, publishOptions, () => {
    ...
    this.enableAnchoringNotifications = (status, options, callback) => {
        ...
        if (status && options.ignoreMounts) {
        ...
        if (!status && (!publishOptions || publishOptions.ignoreMounts)) {
            brickMapController.enableAnchoringNotifications(status);
            return callback();
        }
        ...
        const propagateNotificationSettings = (manifest, callback) => {
            ...
            manifest.getArchiveForPath(mountPoint, (err, result) => {
                ...
                result.archive.enableAnchoringNotifications(status, options, (err) => {
                    if (err) {
