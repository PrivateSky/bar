...
const BarMapController = require('./BarMapController');
...
function Archive(archiveConfigurator) {
    ...
    const initialize = () => {
        ...
        barMapController = new BarMapController({
    ...
    function buildBrickStorageServiceInstance(storageProvider) {
        const instance = new BrickStorageService({
            ...
            brickDataExtractorCallback: (brickMeta, brick) => {
                ...
                const transformParameters = barMapController.getTransformParameters(brickMeta);
    ...
    function computeFileHash(fileBarPath) {
        const hashList = barMapController.getBricksMeta(fileBarPath).map(brickMeta => brickMeta.hash);
     ...
     */
    this.init = (callback) => {
        barMapController.init(callback);
    }
     ...
     */
    this.load = (callback) => {
        barMapController.load(callback);
    };
     ...
     */
    this.getKeySSI = () => {
        return archiveConfigurator.getKeySSI().getIdentifier();
    }
    ...
    this.getFolderHash = (barPath, callback) => {
        barPath = pskPth.normalize(barPath);
        const fileList = barMapController.getFileList(barPath);
    ...
    this.writeFile = (barPath, data, options, callback) => {
        ...
        brickStorageService.ingestData(data, (err, result) => {
            ...
            barMapController.addFile(barPath, result, callback);
        });
    };
    ...
    this.readFile = (barPath, callback) => {
        ...
        try {
            bricksMeta = barMapController.getBricksMeta(barPath);
        } catch (err) {
            return callback(err);
        }
    ...
    this.addFiles = (files, barPath, options, callback) => {
        ...
        brickStorageService.ingestFiles(filesArray, (err, result) => {
            ...
            barMapController.addFiles(barPath, result, callback);
        });
    };
    ...
    this.appendToFile = (barPath, data, callback) => {
        ...
        brickStorageService.ingestData(data, (err, result) => {
            ...
            barMapController.appendToFile(barPath, result, callback);
        });
    };
    ...
    this.extractFolder = (fsFolderPath, barPath, callback) => {
        ...
        const filePaths = barMapController.getFileList(barPath);
    ...
    this.delete = (barPath, callback) => {
        barMapController.deleteFile(barPath, callback);
    };
    ...
    this.rename = (srcPath, dstPath, callback) => {
        ...
        barMapController.renameFile(srcPath, dstPath, callback);
    }
    ...
    this.listFiles = (folderBarPath, options, callback) => {
        ...
        let fileList;
        try {
            fileList = barMapController.getFileList(folderBarPath, options.recursive);
        } catch (e) {
            return callback(e);
        }
    ...
    this.listFolders = (folderBarPath, options, callback) => {
        ...
        callback(undefined, barMapController.getFolderList(folderBarPath, options.recursive));
    };
    ...
    this.createFolder = (barPath, callback) => {
        barMapController.createDirectory(barPath, callback);
    };
    ...
    this.clone = (targetStorage, preserveKeys = true, callback) => {
        targetStorage.getBarMap((err, targetBarMap) => {
            ...
            const fileList = barMapController.getFileList("/");
            const bricksList = {};
            for (const filepath of fileList) {
                bricksList[filepath] = barMapController.getBricksMeta(filepath);
            }

            brickStorageService.copyBricks(bricksList, {
                dstStorage: targetStorage,
                beforeCopyCallback: (brickId, brick) => {
                    const transformParameters = barMapController.getTransformParameters(brickId);
    ...
    this.setValidationRules = (rules) => {
        barMapController.setValidationRules(rules);
    }
    ...
    this.doAnchoring = () => {
        ...
        barMapController.anchorChanges(anchoringEventListener);
    }
