...
function Archive(archiveConfigurator) {
    ...
    this.addFile = (fsFilePath, barPath, isEncrypted, callback) => {
        ...
        function __addFile() {
            createBricks(fsFilePath, barPath, archiveConfigurator.getBufferSize(), isEncrypted, (err) => {
                if (err) {
                    return callback(err);
                }
    ...
    this.addFiles = (arrWithFilePaths, barPath, areEncrypted, callback) => {
        ...
        function recAdd() {
            if (arr.length > 0) {
                ...
                createBricks(filePath, barPath + "/" + fileName, archiveConfigurator.getBufferSize(), areEncrypted,(err) => {
                    if (err) {
                        return callback(err);
                    }
    ...
    this.addFolder = (fsFolderPath, barPath, isEncrypted, callback) => {
        if (typeof isEncrypted === "function") {
            callback = isEncrypted;
            isEncrypted = true;
        }
        ...
        function __addFolder() {
            ...
            function readFileCb(err, file, rootFsPath) {
                ...
                if (typeof file !== "undefined") {
                    ...
                    createBricks(path.join(rootFsPath, file), barPath + "/" + normalizedFilePath, archiveConfigurator.getBufferSize(), isEncrypted, (err) => {
                        if (err) {
                            return callback(err);
                        }
