...
function Archive(name,provider){
    ...
    this.createBricks = function(data){
        var slice = '_';
        while(slice !== ''){
    ...
    var walkFolder = function(folderName,callback){
        fs.readdir(folderName,function(err,files){
            files.forEach(file=>{
                if(fs.statSync((folderName + '/' + file)).isDirectory()){
                    walkFolder((folderName + '/' + file),callback);
                }else{
                    //console.log((folderName + '/' + file));
                    worker.readFile((folderName + '/' +file),callback);
                }
            });
        });
    }
    this.addFolder = function(folderName,callback){
        ...
        walkFolder(folderName,callback);
    }
...
let a = new Archive();
a.addFolder('bar',a.createBricks);