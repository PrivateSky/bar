...
function Archive(archiveConfigurator) {
    ...
    function buildBrickStorageServiceInstance() {
        const instance = new BrickStorageService({
            ...
            brickDataExtractorCallback: (brickId, brick) => {
    ...
    this.readFile = (barPath, callback) => {
        ...
        brickStorageService.createBufferFromBricks(brickIds, (err, buffer) => {
            if (err) {
                return callback(err);
            }
    ...
    this.createReadStream = (barPath, callback) => {
        ...
        brickStorageService.createStreamFromBricks(brickIds, (err, stream) => {
            if (err) {
                return callback(err);
            }
    ...
    this.extractFile = (fsFilePath, barPath, callback) => {
        ...
        brickStorageService.createFileFromBricks(fsFilePath, brickIds, callback);
    };
    ...
    this.clone = (targetStorage, preserveKeys = true, callback) => {
        targetStorage.getBarMap((err, targetBarMap) => {
            ...
            for (const filepath of fileList) {
                bricksList[filepath] = barMap.getHashList(filepath);
            }
