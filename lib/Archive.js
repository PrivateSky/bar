...
function Archive(archiveConfigurator) {
    ...
    this.addFile = (fsFilePath, barPath, options, callback) => {
        ...
        function __addFile() {
            createBricks(fsFilePath, barPath, archiveConfigurator.getBufferSize(), options.isEncrypted, (err) => {
                if (err) {
                    return callback(err);
                }
    ...
    this.addFiles = (arrWithFilePaths, barPath, options, callback) => {
        ...
        function recAdd() {
            if (arr.length > 0) {
                ...
                createBricks(filePath, barPath + "/" + fileName, archiveConfigurator.getBufferSize(), options.isEncrypted,(err) => {
                    if (err) {
                        return callback(err);
                    }
        ...
        function __addFolder() {
            ...
            function readFileCb(err, file, rootFsPath) {
                ...
                if (typeof file !== "undefined") {
                    ...
                    createBricks(path.join(rootFsPath, file), barPath + "/" + normalizedFilePath, archiveConfigurator.getBufferSize(), options.isEncrypted, (err) => {
                        if (err) {
                            return callback(err);
                        }
