...
function Archive(archiveConfigurator) {
    ...
    this.addFolder = (srcPath, destPath, callback) => {
        ...
        if (!destPath) {
            destPath = path.dirname(srcPath);
        }
        loadBarMapThenExecute(__addFolder, callback);

        function __addFolder() {

            archiveFsAdapter.getNextFile(srcPath, readFileCb);
            ...
            function readFileCb(err, file) {
                ...
                if (typeof file !== "undefined") {
                    readFileAsBlocks(path.join(path.dirname(srcPath), file), path.join(destPath, file), archiveConfigurator.getBufferSize(), (err) => {
                        if (err) {
                            return callback(err);
                        }
    ...
    this.addFile = (srcPath, destPath, callback) => {
        if (typeof destPath === "function") {
            callback = destPath;
            destPath = undefined;
        }
        ...
        function __addFile() {
            readFileAsBlocks(srcPath, destPath, archiveConfigurator.getBufferSize(), (err) => {
                if (err) {
                    return callback(err);
                }
    ...
    this.addRawData = (destPath, data, callback) => {
    ...
    this.extractFile = (srcPath, destPath, callback) => {
        if (typeof destPath === "function") {
            callback = destPath;
            destPath = undefined;
        }
        ...
        function __extractFile() {
            const brickIds = barMap.getHashList(srcPath);
            ...
            function getFileRecursively(brickIndex, callback) {
                ...
                storageProvider.getBrick(brickId, (err, brick) => {
                    ...
                    archiveFsAdapter.appendBlockToFile(destPath, brick.getRawData(), (err, updatedFile) => {
                        if (err) {
                            return callback(err);
                        }
                        ...
                        if (brickIndex < brickIds.length) {
                            ...
                            callback(undefined, updatedFile);
                        }
                    });
                });
            }
    ...
    this.extractFolder = (destPath, callback) => {
        if (typeof destPath === "function") {
            callback = destPath;
            destPath = undefined;
        }
        ...
        loadBarMapThenExecute(() => {
            ...
            filePaths.forEach(filePath => {
                this.extractFile(filePath, destPath, (err) => {
                    if (err) {
                        return callback(err);
                    }
    ...
    function readFileAsBlocks(srcPath, destPath, blockSize, callback) {

        archiveFsAdapter.getFileSize(srcPath, (err, fileSize) => {
            if (err) {
                return callback(err);
            }
            ...
            function __readBlocksRecursively(blockIndex, callback) {
                archiveFsAdapter.readBlockFromFile(srcPath, blockIndex, blockSize, (err, blockData) => {
                    if (err) {
                        return callback(err);
                    }
