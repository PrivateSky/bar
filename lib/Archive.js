    ...
    this.extractFile = function (fileName, location, callback) {
        ...
        function helperExtractFile() {
            ...
            filenamesList.forEach(file=> {
                ...
                __getFileRecursively(hashList, hashList.length, 0, file, callback);
            });
        }

        function __getFileRecursively(hashList, length, index, file, callback) {
            // if (index === length) {
            //     return callback();
            // }
            ...
            storageProvider.getBrick(hashList[index], (err, data) => {
                ...
                __appender(err, data, file, (err) => {
                    if (err) {
                        return callback(err);
                    }
                    if(index < length-1) {
                        __getFileRecursively(hashList, length, index + 1, file, callback);
                    }else{
                        return callback();
                    }
                });
        ...
        function __appender(err, data, fileName, callback) {
            if (err) {
                return callback(err);
            }
