...
function Archive(archiveConfigurator) {
    ...
    /**
     * Toggle publishing new anchors notifications
     * @param {boolean} status
     */
    this.toggleAnchoringNotifications = (status) => {
        brickMapController.toggleAnchoringNotifications(status);
    }
     ...
     */
    this.toggleAutoSync = (status, options, callback) => {
        autoSync = status;
        ...
        if (autoSync && !options) {
            return OpenDSUSafeCallback(callback)(createOpenDSUErrorWrapper(`Missing auto sync options`));
        }

        if (autoSync && options) {
            options = {
                ...autoSyncDefaultOptions,
                ...options
            };
        }
        ...
        archiveConfigurator.getKeySSI(undefined, (err, keySSI) => {
            ...
            if (!autoSync) {
                notifications.unsubscribe(keySSI, dsuObsHandler);
                autoSyncOptions = {};
                return;
            }
            ...
            dsuObsHandler.on('error', (err) => {
                autoSyncOptions.onError(err);
            });

            dsuObsHandler.on('message', async (message) => {
                if (!message.ok) {
                    autoSyncOptions.onError(new Error(`Unable to fetch notification. Code: ${message.statusCode}. Message: ${message.statusMessage}`));
                    return;
                }
                ...
                brickMapController.mergeUpstreamChanges((err) => {
                    if (err) {
                        autoSyncOptions.onError(err);
                    }
                })
