...
function Archive(archiveConfigurator) {
    ...
    this.update = (fsPath, callback) => {
        ...
        function __readDirectoryRecursively(folderPath, sign, callback) {
            ...
            function __readFileChk(err, file) {
                ...
                if (typeof file === 'undefined') {
                    return callback(undefined,blocksPositions);
                }
                ...
                archiveFsAdapter.getFileSize(goodPath, (err, size) => {
                    ...
                    __readBlock(goodPath, size, 0, archiveConfigurator.getBufferSize(), undefined, undefined, (err) => {
                        if (err) {
                            return callback(err);
                        }
            ...
            function __readBlock(file, fileSize, index, blockSize, currentBlockCheckSum, firstByte, callback) {
                if (index >= fileSize) {
                    return callback();
                }
                archiveFsAdapter.readBlockFromFile(file, index, index + blockSize - 1 , (err, data) => {
                    ...
                    if(matchFound === false) {
                        __readBlock(file, fileSize, index + 1, blockSize, currentBlockCheckSum, data[0], callback);
                    }else{
                        __readBlock(file, fileSize, index + blockSize, blockSize,undefined, undefined, callback);
                    }
                });
            }
        ...
        function __update() {
            __readDirectoryRecursively(fsPath,  true, callback);
        }
    };
