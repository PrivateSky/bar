...
function Archive(archiveConfigurator,mapDigest) { //configObj
    ...
    function putBarMap(callback)
    {
        ...
        storageProvider.putBarMap((err,newMapDigest)=>{
            if(err)
                return callback(err);
        ...
        if(barMap === undefined)
        {
            storageProvider.getBarMap(mapDigest,(err,map)=>{
                if(err)
                    return callback(err);
                barMap=map;
                __helper_appendToFile();
            });
        }else{
            __helper_appendToFile();
        }

        function __helper_appendToFile(){
    ...
    this.addFolder = function (folderPath, callback) {
        ...
        // storageProvider.getBarMap((err, barMap) => {
        //     if (err) {
        //         return callback(err);
        //     }
        //
        ...
        //             //     callback(err, mapBrick.getHash());
        //             // });
        //             storageProvider.putBarMap(callback);
        //         }
        //     }
        // });
    };
        ...
        if(barMap === undefined){
            storageProvider.getBarMap(mapDigest,(err,map)=>{
                if(err)
                    return callback(err);
                barMap = map;
                __helper_replaceFile();
            });
        }else{
            __helper_replaceFile();
        }

        function __helper_replaceFile(){
    ...
    this.getFile = function (fileName, location, callback) {
        if(barMap === undefined){
            ...
            console.log(barMap.getFileList());
            __helper_getFile();
        }

        function __helper_getFile()
        {
            fileName = validateFileName(fileName);
    ...
    this.getFolder = function (savePath, callback) {
    ...
    this.getReadStream = function (filePath) {
        ...
        // let index = 0;
        // barMap.getHashList(filePath).forEach(fileHash=>{
        //     storageProvider.getBrick(fileHash,(err,brick)=>{
        //         storageProvider.appendToTempFile();
        //     });
        // });
    };
