...
function Archive(archiveConfigurator) {
    ...
    function buildBrickStorageServiceInstance(keySSI, storageProvider) {
        const instance = new BrickStorageService({
            ...
            brickFactoryFunction: (encrypted) => {
                encrypted = (typeof encrypted === 'undefined') ? true : !!encrypted;
                if (encrypted) {
                ...
                let key = SSIKeys.buildSeedSSI(keySSI.getDLDomain(), '', keySSI.getControl(), keySSI.getVn());
