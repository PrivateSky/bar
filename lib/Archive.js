...
function Archive(archiveConfigurator) {
    ...
    const _addFiles = (files, barPath, options, callback) => {
        ...
        const ingestionMethod = (!options.compact) ? 'ingestFiles' :'createBrickFromFiles';

        brickStorageService[ingestionMethod](filesArray, (err, result) => {
            if (err) {
                return callback(err);
            }
    ...
    const _addFolder = (fsFolderPath, barPath, options, callback) => {
        ...
        const ingestionMethod = (!options.compact) ? 'ingestFolder' :'createBrickFromFolder';

        brickStorageService[ingestionMethod](fsFolderPath, (err, result) => {
            if (err) {
                return callback(err);
            }
    ...
    this.addFolder = (fsFolderPath, barPath, options, callback) => {
        const defaultOpts = {encrypt: true, ignoreMounts: false, compact: false};
        if (typeof options === "function") {
            callback = options;
            options = {};
        }
