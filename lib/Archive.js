...
function Archive(name, storageProvider, userProvider){
    ...
    this.appendToFile = function(filePath,stream,callback){
        ...
        stream.on('error',()=>{
            ...
            console.log(barMap.getHeader()[filePath]);
            console.log(tempBrick.getHash());
    ...
    this.getFile = function (fileName,location,callback) {
        const hashList = barMap.getHeader()[fileName];
        __getFileRecursively(hashList,hashList.length,0);
        function __getFileRecursively(hashList,length,index){
            if(index === length){
                return callback();
            }
            storageProvider.getBrick(hashList[index],(err,data)=>{
                ...
                __appender(err,data);
                __getFileRecursively(hashList,length,(index+1));
            });
        }
        function __appender(err,data){
            if(err){
                return callback(err);
            }
            let base = path.basename(fileName);
            let cale = path.join(location,base.toString());
            //console.log(cale);
            userProvider.appendBlockToFile(cale,data,(err)=>{
                //console.log('Doni');
                return callback(err);
            });
        }
    ...
    this.list = function(callback){
        callback(undefined,Object.keys(barMap.getHeader()));
