    ...
    this.extractFolder = function (savePath, callback) {
        ...
        function helperExtractFolder() {
            ...
            function readFilesRecursively(fileIndex, readFilesCb) {
                ...
                function getBricks(brickIndexStart, brickIndexEnd, getBricksCb){

                    console.log(brickIndexStart, brickIndexEnd, brickList.length);

                    if(brickIndexStart>=brickList.length){
                        return getBricksCb();
                    }

                    if(brickIndexEnd>=brickList.length){
                        brickIndexEnd = brickList.length-1;
                    }
                    ...
                    while (brickIndexStart <= brickIndexEnd) {
                        numberOfAvailableBricks++;
                        const brickHash = brickList[brickIndexStart];
                        ...
                        storageProvider.getBrick(brickHash, (err, brickData) => {
                            ...
                            if(numberOfAvailableBricks === 0){
                                appendBlock(queue);
                                getBricks(brickIndexEnd+1, brickIndexEnd+100, getBricksCb);
                            }
                        });
                ...
                /*make sure that the file does not exist before appending bricks to it*/
