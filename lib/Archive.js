...
function Archive(archiveConfigurator) { //configObj
    ...
    this.addFolder = function (folderPath, callback) {
        diskAdapter.getFilesAndFolders(folderPath, (err, rootFolder, listFolders, listFiles) => {
            if (err) {
                return callback(err);
            }

            extractData(rootFolder, listFolders, listFiles, callback);
        });
    };
    ...
    function walkFiles(rootFolder, listFiles, callback) {
        ...
        listFiles.forEach(filePath => {
            readFileAsBlocks(rootFolder, filePath, archiveConfigurator.getBufferSize(), (err) => {
                if (err) {
                    return callback(err);
                }
    ...
    function readFileAsBlocks(rootFolder, filePath, blockSize, callback) {
        diskAdapter.getFileSize(filePath, (err, fileSize) => {
            if (err) {
                return callback(err);
            }
            ...
            function __readCb(err, buffer) {
                ...
                let removablePath = rootFolder.split(path.sep);
                removablePath.pop();
                const removablePathLen = removablePath.join(path.sep).length;
                barMap.add(filePath.substring(removablePathLen), brick.getHash());
                storageProvider.putBrick(brick, (err) => {
                    ...
                    if (iter < noBlocks) {
                        diskAdapter.readBlockFromFile(filePath, iter, blockSize, __readCb);
                    } else {
                        callback();
                    }
    ...
    function walkFolders(rootFolder, listFolders, callback) {
        if (listFolders.length > 0) {
            __walkFoldersRecursively(0, callback);
        } else {
            callback();
        }

        function __walkFoldersRecursively(index, callback) {
            diskAdapter.getFilesAndFolders(listFolders[index], (err, localRootFolder, listFolders, listFiles) => {
                if (err) {
                    return callback(err);
                }
                ++index;
                if (index < listFolders.length) {
                    __walkFoldersRecursively(index, callback);
                } else {
                    extractData(rootFolder, listFolders, listFiles, callback);
                }
            });
    ...
    function extractData(rootFolder, listFolders, listFiles, callback) {
        //process lists
        walkFiles(rootFolder, listFiles, (err) => {
            if (err) {
                return callback(err);
            }

            walkFolders(rootFolder, listFolders, callback);
        });
    }
}
