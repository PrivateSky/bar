...
function Archive(archiveConfigurator) {
    ...
    this.refresh = (callback) => {
        this.load((err) => {
            ...
            this.enableAnchoringNotifications(publishAnchoringNotifications);
            callback();
        });
    }
    ...
    this.enableAutoSync = (status, options, callback) => {
        ...
        archiveConfigurator.getKeySSI(undefined, (err, keySSI) => {
            ...
            if (!status) {
                notifications.unsubscribe(dsuObsHandler);
                return;
            }
            ...
            dsuObsHandler.on('message', async (message) => {
                ...
                const mergeNewDiffs = (batchOperationsInProgress) ? false : true;
                brickMapController.mergeUpstreamChanges(mergeNewDiffs, (err) => {
                    if (err) {
                        options.onError(err);
                    }
                    options.onSync();
