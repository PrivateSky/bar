...
const InternalBarHooks = require('./InternalBarHooks');
...
function Archive(archiveConfigurator) {
    ...
    const anchorVerificationStrategy = archiveConfigurator.getAnchorVerificationStrategy();
    const barHooks = new InternalBarHooks({
        anchorVerificationStrategy
    });
    ...
    const anchoringService = new AnchoringService({
        strategy: anchorVerificationStrategy,
        brickStorageService,
        barMapId: archiveConfigurator.getBarMapId()
    })
    ...
    this.init = (callback) => {
        ...
        barMap.load();
        anchoringService.setBarMap(barMap);
        callback();
    }
     ...
     */
    this.load = (callback) => {
        const alias = archiveConfigurator.getBarMapId();
        getBarMap(alias, (err, map) => {
            if (err) {
                return callback(err);
            }
            barMap = map;
    ...
    this.writeFile = (barPath, data, options, callback) => {
        ...
        brickStorageService.ingestData(data, (err, result) => {
            ...
            this.executeHook('beforeWriteFile', barPath, result, (err) => {
                if (err) {
                    return callback(err);
                }

                this.anchorChange('writeFile', {
                    path: barPath,
                    bricksData: result,
                }, callback);
            })
    ...
    this.addFile = (fsFilePath, barPath, options, callback) => {
        ...
        brickStorageService.ingestFile(fsFilePath, (err, result) => {
            ...
            this.executeHook('beforeAddFile', barPath, result, (err) => {
                if (err) {
                    return callback(err);
                }

                this.anchorChange('addFile', {
                    path: barPath,
                    bricksData: result,
                }, callback);
            })
    ...
    this.addFiles = (files, barPath, options, callback) => {
        ...
        brickStorageService.ingestFiles(filesArray, (err, result) => {
            ...
            this.executeHook('beforeAddFiles', barPath, result, (err) => {
                if (err) {
                    return callback(err);
                }

                this.anchorChange('addFiles', {
                    path: barPath,
                    files: result
                }, callback);
    ...
    this.appendToFile = (barPath, data, callback) => {
        ...
        brickStorageService.ingestData(data, (err, result) => {
            ...
            this.executeHook('beforeAppendToFile', barPath, result, (err) => {
                if (err) {
                    return callback(err);
                }

                this.anchorChange('appendFile', {
                    path: barPath,
                    bricksData: result,
                }, callback);
            });
    ...
    this.addFolder = (fsFolderPath, barPath, options, callback) => {
        ...
        brickStorageService.ingestFolder(fsFolderPath, (err, result) => {
            ...
            this.executeHook('beforeAddFolder', barPath, result, (err) => {
                if (err) {
                    return callback(err);
                }

                this.anchorChange('addFolder', {
                    path: barPath,
                    files: result
                }, callback);
            })
    ...
    this.delete = (barPath, callback) => {
        this.executeHook('beforeDelete', barPath, (err) => {
            if (err) {
                return callback(err);
            }

            this.anchorChange('delete', {
                path: barPath
            }, callback);
        });
    };
     ...
     * @param {...} args
     */
    this.executeHook = (hookName, ...args) => {
        const callback = args[args.length - 1];

        if (barHooks.hasHandler(hookName)) {
            return barHooks.executeHandler(hookName, args);
        }
     ...
     * @param {callback} callback
     */
    this.anchorChange = (changeType, config, callback) => {
        anchoringService.applyChange(changeType, config, callback);
    }
}
