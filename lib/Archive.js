...
function Archive(archiveConfigurator) {
    ...
    this.addAll = (fsPaths, barBasePath, callback) => {
        const fs = require('fs');
        ...
        const __addNext = (index = 0) => {
            if(index >= fsPaths.length) {
                return callback();
            }

            const currentPath = fsPaths[index];
            ...
            fs.stat(currentPath, (err, stats) => {
                ...
                if(stats.isDirectory()) {
                    this.addFolder(currentPath, barBasePath, (err) => {
                        if (err) {
                            return callback(err);
                        }
                    ...
                    this.addFile(currentPath, `${barBasePath}/` + path.basename(currentPath), (err) => {
                        if (err) {
                            return callback(err);
                        }
