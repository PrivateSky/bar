...
assert.callback("testCommandLine",(callback)=>{
    ...
    assert.true(parameters.length === 2 && flags[0].toString() === '-cf' && parameters[0].toString() === 'name.bar' && parameters[1].toString() === 'folderName' && flags.length === 1 && commandHandler.getCommand() === 'cf');
    commandHandler = new CommandHandler(commandLine[1]);
    parameters = commandHandler.getParameters();
    flags = commandHandler.getFlags();
    assert.true(parameters.length === 1 && flags.length === 2 && flags[0].toString() === '-x' && flags[1].toString() === '--file' && parameters[0].toString() === 'name.bar' && commandHandler.getCommand() === 'xf');
    commandHandler = new CommandHandler(commandLine[2]);
    parameters = commandHandler.getParameters();
    flags = commandHandler.getFlags();
    assert.true(parameters.length === 1 && flags.length === 1 && flags[0].toString() === '-tf' && parameters[0].toString() === 'name.bar' && commandHandler.getCommand() === 'tf');
